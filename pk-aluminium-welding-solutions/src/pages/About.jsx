import React, { useEffect } from 'react'
import {
  Box,
  Container,
  Heading,
  Text,
  Stack,
  Image,
  SimpleGrid,
  VStack,
  Button,
  Divider,
  Icon,
  useColorModeValue,
  List, // Added List component
  ListItem, // Added ListItem component
  ListIcon, // Added ListIcon component
} from '@chakra-ui/react'
import { motion, useInView } from 'framer-motion'
import { FaHistory, FaBullseye, FaHandshake, FaToolbox, FaUserTie, FaCheckCircle, FaCarSide, FaLock, FaHome } from 'react-icons/fa' // Added new icons
// Import the SEO component
import SeoHead from '../components/common/SeoHead'

// --- Custom Motion Components (FIXED DEPRECATION) ---
const MotionBox = motion.create(Box)
const MotionHeading = motion.create(Heading)
const MotionText = motion.create(Text)
const MotionImage = motion.create(Image)

// --- Utility Components for the About Page ---

// Component for displaying a core value
const ValueCard = ({ icon, title, description, delay }) => {
  const ref = React.useRef(null)
  const isInView = useInView(ref, { once: true, amount: 0.3 })

  return (
    <MotionBox
      ref={ref}
      p={6}
      bg="white"
      rounded="xl"
      shadow="lg"
      textAlign="center"
      borderBottom="4px solid"
      borderColor="blue.500"
      initial={{ opacity: 0, scale: 0.9 }}
      animate={isInView ? { opacity: 1, scale: 1 } : {}}
      transition={{ duration: 0.6, delay: delay }}
      whileHover={{ scale: 1.03, shadow: 'xl' }}
    >
      <Icon as={icon} w={10} h={10} color="blue.500" mb={4} />
      <Heading size="md" mb={2} color="gray.800">
        {title}
      </Heading>
      <Text color="gray.600" fontSize="sm">
        {description}
      </Text>
    </MotionBox>
  )
}

// Placeholder Team Data - Keep this for now, but focus on the new section
const teamMembers = [
  { name: 'Pieter Kriel', title: 'Founder & Master Welder', img: '/images/team-pieter.jpg', bio: 'Decades of experience in specialized TIG welding and custom design.' },
  { name: 'Sarah Jansen', title: 'Project Director', img: '/images/team-sarah.jpg', bio: 'Manages all architectural projects, ensuring seamless delivery and client satisfaction.' },
  { name: 'Thabo Mbele', title: 'Head of Fabrication', img: '/images/team-thabo.jpg', bio: 'Leads the workshop with a focus on safety, efficiency, and quality control.' },
]

// --- Main About Component ---

export default function About() {
  // Removed document.title useEffect, as SeoHead handles it
  const accentColor = useColorModeValue('blue.600', 'blue.300')

    // --- SEO Metadata for the About Page ---
    // NOTE: Consider updating the SEO description to mention the new product categories (Automotive & Security)
    const seoTitle = "About PK Aluminium | Master Welder & Local Cape Town Expertise";
    const seoDescription = "Learn about PK Aluminium, our master welder Pieter Kriel, and our commitment to TIG welding quality and local expertise in Maitland, Cape Town. Focused on precision, integrity, and custom fabrication for security and automotive solutions.";

  return (
    <Box bg="gray.50" color="gray.800">

        {/* --- SEO IMPLEMENTATION --- */}
        <SeoHead
            title={seoTitle}
            description={seoDescription}
            path="about" // Sets the canonical URL to /about
            ogImage="logo.jpg"
        />
        {/* --- END SEO --- */}

        {/* ------------------------------------- */}
        {/* ## HERO SECTION: About Us Title (WITH BACKGROUND IMAGE) */}
        {/* ------------------------------------- */}
        <Box
          bg="gray.800"
          py={{ base: 20, md: 28 }}
          position="relative"
          overflow="hidden"
          minH={{ base: '300px', md: '400px' }}
        >
          {/* Background Image Container */}
          <Box
            aria-hidden
            position="absolute"
            inset={0}
            // NOTE FOR CLIENT: REPLACE THIS GENERIC IMAGE WITH A REAL, HIGH-QUALITY PHOTO!
            bgImage="url(/welding.webp)"
            bgPos="center"
            bgSize="cover"
            bgAttachment="fixed"
            // Dark Overlay to Ensure Text Readability (Accessibility)
            _after={{
              content: '""',
              position: 'absolute',
              inset: 0,
              bgGradient: 'linear(to-b, blackAlpha.700, blackAlpha.800, blackAlpha.900)',
            }}
          />

          <Container maxW="6xl" zIndex="2" position="relative">
            <MotionHeading
              fontSize={{ base: '4xl', md: '6xl' }}
              fontWeight="extrabold"
              mb={4}
              color="white"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8 }}
            >
              Our Story: Forged in Cape Town
            </MotionHeading>
            <MotionText
              fontSize={{ base: 'lg', md: 'xl' }}
              maxW="3xl"
              color="gray.50"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.2 }}
            >
              We are more than fabricators; we are artisans building the future of aluminium structures on a foundation of <strong>precision, reliability, and local expertise.</strong>
            </MotionText>
          </Container>
        </Box>

        {/* ------------------------------------- */}
        {/* ## INTRODUCTION & ORIGIN STORY */}
        {/* ------------------------------------- */}
        <Container maxW="6xl" py={{ base: 16, md: 24 }}>
          <SimpleGrid columns={{ base: 1, md: 2 }} spacing={12} alignItems="center">
            <MotionBox
              initial={{ opacity: 0, x: -40 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.9 }}
            >
              <Heading mb={4} color={accentColor} size="xl">
                From a Single Spark to a South African Leader
              </Heading>
              <Text mb={6} fontSize="lg" lineHeight="tall">
                PK Aluminium Welding Solutions was founded by <strong>Pieter Kriel</strong> right here in <strong>Maitland, Cape Town</strong>, with a simple belief: that aluminium fabrication could be done better. Tired of seeing shoddy work, Pieter channeled his decades of experience as a master welder into a company defined by <strong>uncompromising quality and technical mastery</strong>.
              </Text>
              <Text color="gray.600">
                Today, we serve a broad clientele—from homeowners needing secure, custom gates to architects requiring complex, large-scale structures. While our operation has grown, our commitment to the meticulous craftsmanship of that very first project remains the same.
              </Text>
            </MotionBox>
            <MotionImage
              // NOTE FOR CLIENT: REPLACE THIS GENERIC IMAGE WITH A REAL, HIGH-QUALITY PHOTO!
              src="/welding.webp"
              alt="PK Aluminium Welding fabrication"
              rounded="2xl"
              shadow="2xl"
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.9, delay: 0.3 }}
            />
          </SimpleGrid>
        </Container>

        <Divider />

        {/* ------------------------------------- */}
        {/* ## MISSION, VISION & CORE VALUES */}
        {/* ------------------------------------- */}
        <Container maxW="7xl" py={{ base: 16, md: 24 }}>
          <MotionHeading
            textAlign="center"
            mb={12}
            color="gray.700"
            size="2xl"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
          >
            Our Guiding Principles
          </MotionHeading>

          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={10}>
            <ValueCard
              icon={FaBullseye}
              title="Precision"
              description="We commit to tolerances measured in millimeters, ensuring every structure is flawless, functional, and fit-for-purpose."
              delay={0}
            />
            <ValueCard
              icon={FaHandshake}
              title="Integrity"
              description="Clear communication, transparent pricing, and standing behind our work are non-negotiable elements of every partnership."
              delay={0.2}
            />
            <ValueCard
              icon={FaToolbox}
              title="Durability"
              description="By using only high-grade aluminium and expert welding, we deliver solutions engineered to withstand the harsh coastal climate."
              delay={0.4}
            />
          </SimpleGrid>

          <Stack spacing={4} align="center" mt={16}>
            <Heading as="h3" size="lg" color={accentColor}>
              Our Vision
            </Heading>
            <MotionText
              maxW="5xl"
              textAlign="center"
              fontSize="xl"
              color="gray.700"
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true, amount: 0.5 }}
              transition={{ duration: 0.7 }}
            >
              To be recognized as the <strong>benchmark for aluminium welding and fabrication excellence</strong> in South Africa, driving innovation in design and manufacturing practices.
            </MotionText>
          </Stack>
        </Container>

        <Divider />

        {/* Stats Band */}
        <Box bg="gray.800" color="white" py={{ base: 12, md: 16 }}>
          <Container maxW="6xl">
            <SimpleGrid columns={{ base: 1, md: 3 }} spacing={8} textAlign="center">
              <MotionBox initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
                <Heading size="2xl" color="blue.300">15+ </Heading>
                <Text color="gray.300">Years of experience</Text>
              </MotionBox>
              <MotionBox initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
                <Heading size="2xl" color="blue.300">350+ </Heading>
                <Text color="gray.300">Projects delivered</Text>
              </MotionBox>
              <MotionBox initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }}>
                <Heading size="2xl" color="blue.300">98% </Heading>
                <Text color="gray.300">On‑time completion</Text>
              </MotionBox>
            </SimpleGrid>
          </Container>
        </Box>

        {/* ------------------------------------- */}
        {/* ## OUR EXPERTISE & CAPABILITIES (MODIFIED SECTION) */}
        {/* ------------------------------------- */}
        <Container maxW="7xl" py={{ base: 16, md: 24 }}>
          <MotionHeading
            textAlign="center"
            mb={12}
            color="gray.700"
            size="2xl"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
          >
            Our Expertise: Custom Fabrication Made Local
          </MotionHeading>
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={8}>
            {/* NEW COLUMN: Automotive & Outdoor Solutions */}
            <MotionBox
              p={8}
              bg="white"
              rounded="2xl"
              shadow="xl"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.6 }}
            >
              <Heading size="md" mb={4} color={accentColor}>
                <Icon as={FaCarSide} mr={2} /> Automotive & Outdoor
              </Heading>
              <List spacing={3} color="gray.700">
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Custom <strong>Cattle Rails</strong> and Roll Bars
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Durable <strong>Bakkie Drawer Systems</strong>
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Robust <strong>Roof Racks</strong> and Tent Mounting Systems
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Lightweight and reliable <strong>Trailers</strong> (Custom builds)
                </ListItem>
              </List>
            </MotionBox>

            {/* NEW COLUMN: Security & Architectural Fabrication */}
            <MotionBox
              p={8}
              bg="white"
              rounded="2xl"
              shadow="xl"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.6, delay: 0.15 }}
            >
              <Heading size="md" mb={4} color={accentColor}>
                <Icon as={FaLock} mr={2} /> Security & Architectural
              </Heading>
              <List spacing={3} color="gray.700">
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  High-security <strong>Burglar Bars</strong> (Burglars)
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Residential & Commercial <strong>Gates</strong> (Sliding/Swing)
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Precision-built <strong>Window Frames</strong>
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Custom <strong>Fencing</strong> and Security Structures
                </ListItem>
              </List>
            </MotionBox>

            {/* MODIFIED COLUMN: Core Workshop Capabilities */}
            <MotionBox
              p={8}
              bg="white"
              rounded="2xl"
              shadow="xl"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.6, delay: 0.3 }}
            >
              <Heading size="md" mb={4} color={accentColor}>
                <Icon as={FaToolbox} mr={2} /> Core Fabrication Process
              </Heading>
              <List spacing={3} color="gray.700">
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  TIG and MIG welding for aluminium assemblies
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Cutting, bending, and jigging for repeatable accuracy
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  Surface prep and finishing (ready for powder coating)
                </ListItem>
                <ListItem>
                  <ListIcon as={FaCheckCircle} color="green.500" />
                  In‑house prototyping and fit testing
                </ListItem>
              </List>
            </MotionBox>
          </SimpleGrid>
        </Container>

        <Divider />

        {/* ## CTA SECTION: Trust Builder */}
        <Box bg="blue.700" color="white" py={20} textAlign="center">
          <MotionHeading
            size="2xl"
            mb={4}
            initial={{ opacity: 0, scale: 0.8 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
          >
            Ready to Work with a Company You Trust?
          </MotionHeading>
          <MotionText
            fontSize="xl"
            mb={8}
            color="blue.100"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.2 }}
          >
            We’re centrally located in Maitland, serving all of Cape Town. Let's discuss your project.
          </MotionText>
          <Button
            as="a"
            href="/contact"
            size="lg"
            bg="white"
            color="blue.700"
            rounded="full"
            px={12}
            height="55px"
            fontSize="xl"
            _hover={{ bg: 'gray.100', transform: 'translateY(-2px)' }}
            shadow="2xl"
          >
            Get in Touch
          </Button>
        </Box>
    </Box>
  )
}